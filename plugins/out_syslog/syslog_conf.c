...
static flb_sds_t normalize_ra_key_name(struct flb_record_accessor *ra)
{
    int c = 0;
    flb_sds_t name;
    struct mk_list *head;
    ...
    if (ctx->sd_keys) {
        ...
        flb_config_map_foreach(head, mv, ctx->sd_keys) {
            ...
            if (sk_key_ra->ra_sd_key == NULL) {
                flb_plg_error(ins, "could not create record accessor for SD Key %s", mv->val.str);
                return NULL;
            }
            sk_key_ra->key_normalized = normalize_ra_key_name(sk_key_ra->ra_sd_key);
